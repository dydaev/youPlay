...
const PlayListManager = ({
	...
	const handleAddItem = (newItem: listOfPlaylistItemType) => {
		handleSavePlaylistToStorage(newItem);
	...
	const handleUpdateItem = async (newItem: listOfPlaylistItemType, indexOfItem: number) => {
		if (newItem.id) {
			await db.updateData("playLists", { id: newItem.id }, newItem);
			onGetPleyListFromStorage();
		} else {
			onSetList([
